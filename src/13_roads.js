...
##############################`),
  ...
  // A true SEA CROSSING: storm water spans the whole strait (water:true). Falling in
  // is a SPLASH and a bouncy wave-toss (the current shoves you back a step) — never a
  // pit. Sharks cruise UNDER the waves and LEAP in arcs, jellyfish bob ON the surface,
  // urchin-mines ('Z') ride the swell, and a waterspout ('V') gushes on one island.
  L.squall = {
    name: 'THE SQUALL STRAIT', theme: 'squall', boss: 'galegull', gravity: 1, water: true,
    intro: 'A STORM ON THE STRAIT! The sea is WILD — bounce the waves, hop the piers, mind LEAPING SHARKS... and beat the GALE GULL!',
    rows: roadJoin([
roadSeg(`
..............
..............
...
const SideScroll = {
  ...
  mkEnemy(S, ch, i, j, theme) {
    ...
    const e = { kind: ch, species, x: i * ROAD_TILE + 8, y: restY, vx: ch === 'w' ? -44 : 0, vy: 0,
      t: Math.random() * 2, state: 'live', squash: 0, homeY: restY, dashT: 0 };
    if (S.def.water) {                          // SEA ROAD: 'h' swims + leaps, 'f' bobs on the surface
      if (ch === 'h') { e.y = e.homeY = 12 * ROAD_TILE + 22; e.hx = e.x; e.vx = 26; e.leapV = 330 + ((i * 53) % 3) * 38; }
      if (ch === 'f') { e.y = e.homeY = 12 * ROAD_TILE; }
    }
    return e;
  },
  splashFX(S, x) {
    Particles.burst(x, 12 * ROAD_TILE, 'sparkle');
    Particles.burst(x, 12 * ROAD_TILE - 3, 'dust');
  },
  tile(S, i, j) {
  ...
  update(dt, presses) {
    ...
    // STORMY SEA (water roads): falling in means a big SPLASH and a bouncy wave-toss —
    // never a doom pit. The current shoves you back a step, so piers still matter,
    // and the splash refills your flaps like a landing.
    if (def.water && p.y > 12 * ROAD_TILE + 8) {
      p.y = 12 * ROAD_TILE + 8; p.onG = false; p.coyote = 0;
      p.vy = -338; p.vx = Math.min(p.vx, 40) - 30;
      p.flaps = ROAD_FLAPS; p.squash = 0.1;
      S.shake = Math.max(S.shake, 0.06);
      Audio2.jingle('dive'); this.splashFX(S, p.x);
    }
    // pit fall
  ...
  updateEnemies(S, dt) {
    ...
    for (const e of S.enemies) {
      ...
      } else if (e.kind === 'h' && S.def.water) { // SEA LEAPER: cruises under the waves, then BURSTS out
        if (e.leap) {
          e.vy += 700 * dt; e.y += e.vy * dt; e.x += e.vx * dt;
          e.x = clamp(e.x, 8, S.def.w * ROAD_TILE - 8);
          if (e.vy > 0 && e.y >= e.homeY) {       // splashdown: back under, cruise home
            e.y = e.homeY; e.leap = false; e.t = 0;
            e.vx = (Math.abs(e.x - e.hx) > 4 ? Math.sign(e.hx - e.x) : 1) * 26;
            this.splashFX(S, e.x);
          }
        } else {
          e.y = e.homeY + Math.sin(e.t * 2.6) * 2;                 // a lurking shadow under the surf
          e.x += e.vx * dt;
          if (Math.abs(e.x - e.hx) > 42) { e.x = e.hx + Math.sign(e.x - e.hx) * 42; e.vx *= -1; }
          if (e.t > 1.9 && Math.abs(p.x - e.x) < 230) {
            e.leap = true; e.vy = -e.leapV; e.vx = clamp(p.x - e.x, -64, 64) * 0.8;
            this.splashFX(S, e.x); Audio2.jingle('step');
          }
        }
      ...
      } else if (e.kind === 'f' && S.def.water) { // surface bobber: a drifting stinger ON the waves
        e.y = e.homeY - 3 + Math.sin(e.t * 1.9) * 5;
        const nx = e.x + clamp(p.x - e.x, -22, 22) * dt;   // drift toward Noah, but never onto dry land
        if (!ROAD_SOLID[this.tile(S, Math.floor(nx / ROAD_TILE), 12)]) e.x = nx;
  ...
  updateHazards(S, dt) {
    ...
    for (const z of S.bobs) {
      z.t += dt;
      const amp = S.def.water ? 26 : 38;          // sea mines ride the swell, sparks swing wide
      const zx = z.i * ROAD_TILE + 8, zy = z.j * ROAD_TILE + 8 + Math.sin(z.t * 1.7) * amp;
  ...
  drawHazards(c, S) {
    ...
    for (const z of S.bobs) {
      const amp = S.def.water ? 26 : 38;
      const zx = z.i * ROAD_TILE + 8, zy = z.j * ROAD_TILE + 8 + Math.sin(z.t * 1.7) * amp;
      if (S.def.water) {                          // URCHIN-MINE: a spiky drifter riding the swell
        c.strokeStyle = '#241a33'; c.lineWidth = 2;
        for (let k = 0; k < 8; k++) {
          const a = k * Math.PI / 4 + Math.sin(z.t) * 0.15;
          c.beginPath(); c.moveTo(zx + Math.cos(a) * 4, zy + Math.sin(a) * 4);
          c.lineTo(zx + Math.cos(a) * 9, zy + Math.sin(a) * 9); c.stroke();
        }
        c.lineWidth = 1;
        c.fillStyle = '#241a33'; c.beginPath(); c.arc(zx, zy, 6, 0, 7); c.fill();
        c.fillStyle = '#4a3a60'; c.beginPath(); c.arc(zx, zy, 4.6, 0, 7); c.fill();
        c.fillStyle = '#8a6ca8'; c.beginPath(); c.arc(zx - 1.4, zy - 1.4, 1.8, 0, 7); c.fill();
        continue;
      }
  ...
  draw(c) {
    ...
    if (def.water) {                    // THE SEA fills the whole strait; islands draw over it
      const wy = 12 * ROAD_TILE;
      const gg = c.createLinearGradient(0, wy, 0, ROAD_H * ROAD_TILE + 24);
      gg.addColorStop(0, '#2e6096'); gg.addColorStop(1, '#152c54');
      c.fillStyle = gg; c.fillRect(S.camX - 8, wy + 1, SW + 16, ROAD_H * ROAD_TILE - wy + 24);
    }
    ...
    if (def.water) this.drawWaterSurface(c, S, i0, i1);   // translucent surf OVER swimmers = "underwater"
    c.restore();
    this.drawHUD(c, S);
  ...
  // the animated surf: per-column so islands stay dry; crest + foam + edge-wash.
  // Drawn AFTER entities, so anything below the crest reads as submerged.
  drawWaterSurface(c, S, i0, i1) {
    const wy = 12 * ROAD_TILE, bot = ROAD_H * ROAD_TILE + 24;
    const crest = (x) => wy + Math.sin(x * 0.052 + S.t * 2.7) * 2.6;
    const wet = (i) => !ROAD_SOLID[this.tile(S, i, 12)];
    c.save();
    for (let i = i0; i <= i1; i++) {
      if (!wet(i)) {                              // island column: just a lapping edge-wash
        c.globalAlpha = 0.5; c.fillStyle = '#eaf6ff';
        if (wet(i - 1)) c.fillRect(i * ROAD_TILE - 2, wy - 1 + Math.sin(S.t * 3 + i) * 1.5, 4, 2);
        if (wet(i + 1)) c.fillRect((i + 1) * ROAD_TILE - 2, wy - 1 + Math.sin(S.t * 3.4 + i) * 1.5, 4, 2);
        continue;
      }
      const x0 = i * ROAD_TILE;
      c.globalAlpha = 0.34; c.fillStyle = '#7fc4ee';
      c.beginPath(); c.moveTo(x0, bot);
      for (let x = x0; x <= x0 + ROAD_TILE; x += 4) c.lineTo(x, crest(x));
      c.lineTo(x0 + ROAD_TILE, bot); c.closePath(); c.fill();
      c.globalAlpha = 0.75; c.strokeStyle = '#eaf6ff'; c.lineWidth = 1.5;
      c.beginPath();
      for (let x = x0; x <= x0 + ROAD_TILE; x += 4) { const yy = crest(x); if (x === x0) c.moveTo(x, yy); else c.lineTo(x, yy); }
      c.stroke();
      if (((i * 7 + (S.t * 1.3 | 0)) % 5) === 0) { // drifting foam flecks
        c.globalAlpha = 0.55; c.fillStyle = '#ffffff';
        c.fillRect(x0 + ((i * 11 + S.t * 24) % 14), crest(x0 + 8) - 1.5, 2.5, 1.5);
      }
    }
